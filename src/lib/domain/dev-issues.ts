 ...
 * 「急かす」ための材料(滞留日数・未承諾・ヒアリングの放置)もここで算出する。
 */
...
export interface DevIssueUrgency {
  ...
  /** 特に急ぐべきか(不具合の高優先 / 長期滞留 / ヒアリング返信の放置) */
  urgent: boolean;
}
...
export function devIssueShareSummary(
  issue: Pick<DevIssue, "issueNumber" | "title" | "category" | "priority" | "status">,
): string {
  const meta = [
  ...
  return `#${issue.issueNumber} ${issue.title}（${meta}）`;
}
