...
16. （任意）`seed.sql` … 初期データ（自社情報・料金プラン）
...
> `0019` は開発スケジュール（`/dev/schedule`）用です。適用時に、氏名に「酒井」「若林」を含むアカウントへ**閲覧のチェックを自動で付けます**（以降はアプリの「この表を見られるメンバー」で増減できます）。未適用のあいだは管理者だけが閲覧でき、他のメンバーには表示されません。
