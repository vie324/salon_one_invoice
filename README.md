...
デモモードは Supabase 未設定時に自動で有効化され、インメモリのデータで動作します（データはプロセス内のみ・再起動でリセット）。右上の「デモ」セレクトで **全体管理者／請求管理のみ／開発進捗のみ** の3種のアカウントを切り替えられます。
...
- デモモードでは右上の「デモ」セレクトで各種別のアカウントを切り替えて動作を確認できます。
