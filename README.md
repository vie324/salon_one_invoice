...
| **開発進捗（開発依頼・エラー報告）** | スプレッドシート運用を置き換える依頼管理。記載日・依頼者はアカウントから自動記録、分類（不具合/要望）・優先度（高/中/低）・ステータス（未対応/対応中/追加ヒアリング/対応完了）、エンジニアの完了予定日・完了日・開発対応内容、**要望はプロダクト管理者2名の承諾で「実行」**（1名でも停止で「実行なし」）。 |
