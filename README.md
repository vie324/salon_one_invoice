...
| **申込（申込用URL）** | お客様に渡す**申込URLを発行**（宛先ごと・有効期限つき・受付停止可）。お客様はログイン不要でフォームに入力し、内容がそのままツールに反映されます。入力項目は法人名（個人名）・住所（登記住所）・代表者役職・代表者名・**HPB / minimo / EPARK の連携情報（任意・ID＋PASS）**・**LINE連携申込（チェック）**。預かった ID・パスワードは **AES-256-GCM で暗号化して保存**し、画面ではマスク表示（連携作業時に「表示する」で復号）。ワンクリックで**顧客として登録**できます。 |
...
2. **お客様が入力** — `/apply/<token>` のフォームで、法人名（個人の場合は個人名）・住所（法人は登記住所）・代表者役職・代表者名を入力。ホットペッパービューティ / minimo / EPARK の連携情報（ID・PASS）は任意、LINE連携はチェックボックスで申込の有無を選択します。
3. **ツールに反映** — 送信と同時に申込一覧へ表示され、ダッシュボードの「最近の動き」にも記録されます。
4. **顧客として登録** — 申込詳細の「顧客として登録」で顧客レコードを作成し、申込のステータスが「顧客登録済」になります。対応が不要な申込は「対応不要」に変更できます。
