...
3. `.env.example` を `.env.local` にコピーし、値を設定。
   ...
   ```
4. ユーザーを Supabase Auth で作成。サインアップ時に `profiles` が自動作成されます（`role` 既定は `staff`＝請求管理相当）。最初の1人を `profiles.role = 'admin'`（全体管理者）に更新すると、以降は設定画面からアカウント作成・種別変更ができます。
...
- ロールは `profiles.role` で管理します。設定画面（全体管理者のみ）からアカウントの作成・種別変更ができます。
- アカウント作成には `SUPABASE_SERVICE_ROLE_KEY`（サービスロール）が必要です。
- デモモードでは右上の「デモ」セレクトで3種のアカウントを切り替えて動作を確認できます。
