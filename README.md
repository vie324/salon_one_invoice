...
- **実行有無**: 要望はプロダクト管理者（全体管理者）2名の承諾で「実行」、どちらか1名の停止で「実行なし」。
- **通知**: 対応完了で依頼者・管理者に通知（未読のあいだトップにバナー表示）。新規依頼・追加ヒアリング・実行判定も通知されます。
- **スクリーンショット添付**: 依頼フォーム・詳細画面からファイル選択、または画面上で `Ctrl+V`（⌘+V）貼り付け。画像は自動で縮小してアップロードされます（本番は Supabase Storage・非公開バケット＋署名付きURL）。
